	 ...
	 @Override
	    ...
	    protected void configure(HttpSecurity http) throws Exception {
	        http.authorizeRequests().antMatchers("/").permitAll().antMatchers("/welcome")
	            .hasAnyRole("USER", "ADMIN").antMatchers("/listEmployees.html").hasAnyRole("USER", "ADMIN")
	            .antMatchers("/addNewEmployee.html").hasAnyRole("ADMIN").anyRequest().authenticated().and().formLogin()
	    ...
	    public void configureGlobal(AuthenticationManagerBuilder authenticationMgr) throws Exception {
	        authenticationMgr.inMemoryAuthentication().withUser("employee").password("employee")
	            .authorities("ROLE_USER").and().withUser("javainuse").password("	")
